...
/**
 * FileList component displays and manages the user's files and folders.
 *
 * Props:
 * - userId: string - The ID of the current user (used for API requests).
 * - refreshTrigger?: number - Optional. When this value changes, the file list is refreshed.
 * - onFolderChange?: (folderId: string | null) => void - Optional. Callback when the current folder changes.
 */
...
export default function FileList({
  ...
  // State for all files/folders in the current view
  const [files, setFiles] = useState<FileType[]>([]);
  // Loading state for API requests
  const [loading, setLoading] = useState(true);
  // Which tab is active: "all", "starred", or "trash"
  const [activeTab, setActiveTab] = useState("all");
  // The currently open folder's ID (null for root)
  ...
  // Modal states for delete and empty trash confirmation
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [emptyTrashModalOpen, setEmptyTrashModalOpen] = useState(false);
  // The file selected for deletion
  const [selectedFile, setSelectedFile] = useState<FileType | null>(null);

  /**
   * Fetches files/folders for the current user and folder.
   * Uses the userId and currentFolder to build the API URL.
   */
  ...
  /**
   * Filters files based on the active tab.
   * - "all": all files not in trash
   * - "starred": starred files not in trash
   * - "trash": trashed files
   */
   ...
   * @param fileId - The ID of the file to star/unstar.
   */
  ...
  /**
   * Toggle the trashed status of a file.
   * @param fileId - The ID of the file to move to trash or restore.
   */
   ...
   * For images, uses ImageKit's optimized URL; for others, uses fileUrl.
  ...
  const handleDownloadFile = async (file: FileType) => {
    try {
      ...
      if (file.type.startsWith("image/")) {
        // Download image via ImageKit with original quality
        ...
        // Download other file types via fileUrl
  ...
  /**
   * Opens an image in a new tab with optimized view.
  ...
  /**
   * Navigates into a folder and updates the breadcrumb path.
   * @param folderId - The ID of the folder to navigate into.
  ...
  /**
   * Navigates up one level in the folder path.
   */
  const navigateUp = () => {
  ...
  /**
   * Navigates to a specific folder in the breadcrumb path.
   * @param index - The index in the folderPath array.
   */
  ...
  /**
   * Handles clicking on a file or folder row.
   * - If folder: navigates into it.
   * - If image: opens image viewer.
   * @param file - The file/folder object clicked.
   */
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Action buttons (refresh, empty trash, etc.) */}
      <FileActionButtons
        activeTab={activeTab}
        trashCount={trashCount}
        folderPath={folderPath}
